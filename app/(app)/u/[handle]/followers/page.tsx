...
export default async function FollowersPage({ params }: Props) {
  ...
  const visible = rows.filter((r) => !hidden.has(r.follower.id));
      ...
      {visible.length === 0 ? (
        ...
        <ul className="divide-y divide-white/5 rounded-2xl border border-border/50 bg-card/40">
          {visible.map(({ follower }) => (
            <li key={follower.id}>
              <Link
                href={`/u/${encodeURIComponent(follower.handle)}`}
                className="flex items-center gap-3 px-4 py-3 transition hover:bg-muted/15"
              >
                <Avatar className="h-10 w-10 border border-white/10">
                  <AvatarImage src={follower.avatarUrl ?? follower.image ?? undefined} alt="" />
                  <AvatarFallback className="text-xs">
                    {(follower.name ?? follower.handle).slice(0, 2).toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <div className="min-w-0 flex-1">
                  <p className="truncate font-medium text-foreground">
                    {follower.name?.trim() || `@${follower.handle}`}
                  </p>
                  <p className="truncate text-xs text-muted-foreground">@{follower.handle}</p>
                </div>
              </Link>
            </li>
          ))}
